...
final class CarpenterRouterRuntime {
  ...
  void push(YxRoute route, {Map<String, String>? arguments, Map<String, Object?>? extra}) =>
...
final class CarpenterRouteRenderer extends StatelessWidget {
  const CarpenterRouteRenderer({super.key, required this.routes, this.missingRouteBuilder});
  final List<CarpenterRoute> routes;
  final Widget Function(BuildContext context, RouteNode? node)? missingRouteBuilder;
  ...
  Widget build(BuildContext context) {
    ...
    Widget child = terminal.route.page?.call(CarpenterRouteContext(runtime: runtime, match: terminal, chain: chain)) ?? _missing(context, terminal.node);
    for (final match in chain.reversed) {
      final routeContext = CarpenterRouteContext(runtime: runtime, match: match, chain: chain);
  ...
  Widget _missing(BuildContext context, RouteNode? node) => missingRouteBuilder?.call(context, node) ?? Center(child: Text(node == null ? 'Carpenter route tree is empty' : 'Carpenter route is not declared: ${node.route.id}'));
}

List<CarpenterRouteMatch> _matchActiveChain(RouteNode root, List<CarpenterRoute> routes) {
  ...
  while (true) {
    ...
    matches.add(CarpenterRouteMatch(node: node, route: declaration, depth: depth));
