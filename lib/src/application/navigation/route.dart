...
typedef CarpenterRouteScopeBuilder = Widget Function(CarpenterRouteContext context, Widget child);
typedef CarpenterRouteShellBuilder = Widget Function(CarpenterRouteContext context, Widget child);
typedef CarpenterRoutePageBuilder = Widget Function(CarpenterRouteContext context);

/// Carpenter declaration layered over yx_navigation route identity.
final class CarpenterRoute {
  const CarpenterRoute({required this.route, this.scope, this.shell, this.page, this.children = const []});
  final YxRoute route;
...
final class CarpenterRouteMatch {
  const CarpenterRouteMatch({required this.node, required this.route, required this.depth});
...
final class CarpenterRouteContext {
  const CarpenterRouteContext({required this.runtime, required this.match, required this.chain});
