...
typedef CarpenterPageSurfaceOpener = Future<Object?> Function(Object destination);
...
final class _CarpenterSurfaceHostState extends State<CarpenterSurfaceHost> implements CarpenterSurfaceController {
  _SurfaceRequest? _request;

  @override
  Future<T?> openInline<T>(WidgetBuilder builder) => _open<T>(CarpenterSurfaceKind.inline, builder);
  @override
  Future<T?> openSidePanel<T>(WidgetBuilder builder) => _open<T>(CarpenterSurfaceKind.sidePanel, builder);
  ...
  Future<T?> openPage<T>(Object destination) async {
    ...
    if (opener == null) throw StateError('CarpenterSurfaceHost.openPage is not configured.');
  ...
  Widget build(BuildContext context) => CarpenterSurfaceScope(
    ...
    child: LayoutBuilder(
      builder: (context, constraints) {
        ...
        final fullPage = narrow || request.kind == CarpenterSurfaceKind.fullPage;
        final minWidth = constraints.maxWidth < 320 ? constraints.maxWidth : 320.0;
        final width = request.kind == CarpenterSurfaceKind.inline
            ? constraints.maxWidth.clamp(minWidth, 760).toDouble()
            : widget.sidePanelWidth.clamp(minWidth, constraints.maxWidth).toDouble();
            ...
            child: Stack(
              ...
              children: [
                ...
                GestureDetector(
                  ...
                  onTap: _close,
                  child: ColoredBox(color: CarpenterTheme.of(context).overlay.scrim),
                ),
                Align(
                  alignment: fullPage ? Alignment.center : request.kind == CarpenterSurfaceKind.inline ? Alignment.center : widget.sidePanelAlignment,
                  child: SizedBox(
                    width: fullPage ? constraints.maxWidth : width,
                    height: constraints.maxHeight,
                    child: CarpenterSurfaceCloseScope(close: _close, child: Builder(builder: request.builder)),
                  ),
                ),
              ],
            ),
  ...
  void dispose() {
    final request = _request;
    if (request != null && !request.completer.isCompleted) request.completer.complete();
    super.dispose();
  }
}

final class CarpenterSurfaceScope extends InheritedWidget {
  const CarpenterSurfaceScope({super.key, required this.controller, required super.child});
  final CarpenterSurfaceController controller;
  static CarpenterSurfaceController of(BuildContext context) {
    final scope = context.dependOnInheritedWidgetOfExactType<CarpenterSurfaceScope>();
    assert(scope != null, 'No CarpenterSurfaceScope found in context.');
    return scope!.controller;
  }
  @override
  bool updateShouldNotify(CarpenterSurfaceScope oldWidget) => controller != oldWidget.controller;
}
...
final class CarpenterSurfaceCloseScope extends InheritedWidget {
  const CarpenterSurfaceCloseScope({super.key, required this.close, required super.child});
  final void Function([Object? result]) close;
  static bool maybeClose(BuildContext context, [Object? result]) {
    final scope = context.dependOnInheritedWidgetOfExactType<CarpenterSurfaceCloseScope>();
