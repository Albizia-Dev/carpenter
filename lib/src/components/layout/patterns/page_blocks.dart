          ...
          child: Column(
            ...
            children: [
              ...
              if (metadata.isNotEmpty) ...[
                ...
                Wrap(spacing: gap, runSpacing: gap / 2, children: metadata),
              ],
            ],
          ),
        ),
    ...
    return Column(
      ...
      children: [
        for (var index = 0; index < children.length; index++) ...[
          children[index],
          if (index < children.length - 1) SizedBox(height: gap / 2),
        ],
        if (tags.isNotEmpty) ...[
          if (children.isNotEmpty) SizedBox(height: gap / 2),
          Wrap(spacing: gap, runSpacing: gap / 2, children: tags),
        ],
      ],
    );
  }
...
final class CarpenterBlockGroup extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final gap =
        spacing ?? context.units(CarpenterTheme.of(context).spacing.medium);
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        for (var index = 0; index < children.length; index++) ...[
...
final class _CarpenterControllerHostState<C extends ChangeNotifier>
...
final class CarpenterDialogCollectionBody extends StatelessWidget {
  const CarpenterDialogCollectionBody({
    ...
    this.unavailableMessage = 'Data unavailable',
    this.footer,
  });
...
final class _CarpenterTextFieldsDialogState
    extends State<CarpenterTextFieldsDialog> {
  ...
  Widget build(BuildContext context) => CarpenterDialog(
    ...
    actions: [
      CarpenterActionDescriptor(
        ...
        onInvoke: () => widget.onOpenChanged(false),
      ),
      CarpenterActionDescriptor(
        id: 'prompt.submit',
