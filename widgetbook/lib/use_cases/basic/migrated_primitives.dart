...
final colorPickerComponent = WidgetbookComponent(
  ...
  useCases: [
    WidgetbookUseCase(
      name: 'Interactive',
      builder: (_) => const _ColorPickerPreview(),
    ),
  ],
);
...
final dateInputComponent = WidgetbookComponent(
  ...
  useCases: [
    WidgetbookUseCase(
      name: 'Interactive',
      builder: (_) => const _DateInputPreview(),
    ),
  ],
);
...
Widget _activity(BuildContext context) {
  ...
  return previewColumn([
    ...
    SizedBox(
      width: 320,
      child: CarpenterProgress(value: value, semanticLabel: 'Upload progress'),
    ),
...
Widget _activityCompact(BuildContext context) => previewColumn([
  const SizedBox(
    width: 16,
    height: 16,
    child: CarpenterLoader(size: 16, strokeWidth: 2),
  ),
            ...
            children: [
              Expanded(
                child: CarpenterText.body(
                  'Current: ${carpenterFormatRgbHex(_value)}',
                ),
              ),
              CarpenterButton(
                label: _enabled ? 'Disable' : 'Enable',
                prominence: ActionProminence.outlined,
                onInvoke: () => setState(() => _enabled = !_enabled),
              ),
            ],
          ),
        ],
